...
import { getPackagesByCustomer, getFutianOrdersByCustomer, getAllShippingMethods, getActiveWarehouse } from '../../database/pg_service';
...
router.post('/message', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (lower.includes('package') || lower.includes('طرد') || lower.includes('شحنة') || lower.includes('track')) {
      ...
      const summary = pkgs.map((p: any) => `• Box #${p.box_number} (Tracking: ${p.tracking_number}, Status: ${p.status}, Weight: ${p.weight_kg} kg)`).join('\n');
      return res.json({
    ...
    if (lower.includes('futian') || lower.includes('order') || lower.includes('طلب') || lower.includes('فوتيان')) {
      const orders = await getFutianOrdersByCustomer(customerId);
      ...
      const summary = orders.slice(0, 5).map((o: any) => `• ${o.order_id}: ${o.product_name_en} - Status: ${o.status} ($${o.total_price_usd})`).join('\n');
      return res.json({
    ...
    if (lower.includes('rate') || lower.includes('price') || lower.includes('سعر') || lower.includes('تكلفة') || lower.includes('shipping')) {
      ...
      const summary = rates.map((r: any) => `• ${r.title_en} (${r.estimated_days}): $${r.rate_per_kg}/kg (Base fee: $${r.base_fee})`).join('\n');
      return res.json({
